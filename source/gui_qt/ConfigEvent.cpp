...
ConfigEvent::ConfigEvent(MainWindow* parent) : QDialog(parent), ui(new Ui::ConfigEvent) { ui->setupUi(this); }

ConfigEvent::~ConfigEvent() { delete ui; }
...
void ConfigEvent::actApply() { SaveFile(); }

void ConfigEvent::actCancel() { reject(); }
...
void ConfigEvent::actImportRules() {
	...
	gameEvent.setRules(std::string{(std::istreambuf_iterator<char>(file)), (std::istreambuf_iterator<char>())});
	file.close();
	updateDisplay();
}
