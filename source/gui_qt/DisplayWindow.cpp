...
const std::unordered_map<DisplayWindow::Page, int> DisplayWindow::PageIndex = {
		{Page::MainTitle, 0},    {Page::PricesDisplay, 1}, {Page::NumberDisplay, 2}, {Page::EventStarting, 7},
		{Page::PauseDisplay, 4}, {Page::RulesDisplay, 6},  {Page::EventEnding, 5},   {Page::RoundEnding, 3},
};

DisplayWindow::DisplayWindow(core::Event* evt, MainWindow* parent)
	: QMainWindow(parent), ui(new Ui::DisplayWindow), timer(new QTimer(this)), event(evt), mwd(parent) {
	ui->setupUi(this);
...
void setPixMap(QLabel* where, const QString& name, const QImage& img) {
	if (const QString realName = name + "_" + QString::number(where->width()) + "_" + QString::number(where->height());
		where->text() != realName) {// charge l’image seulement une fois
...
void DisplayWindow::updatePauseScreen() const {
	...
	std::vector<std::filesystem::path> slides;
	const std::vector<std::filesystem::path> extension{".png", ".jpg", ".jpeg", ".svg",
													   ".PNG", ".JPG", ".JPEG", ".SVG"};
	...
	if (static_cast<double>(std::chrono::duration_cast<std::chrono::milliseconds>(core::clock::now() - tp).count()) /
				1000.0 <
		dDelay)
		return;
	tp = core::clock::now();
...
void DisplayWindow::resize() {
	...
	if (mwd == nullptr) {
		log_warn("No mainwindows link for Resize!!");
		return;
	}
