...
void ConfigGameRounds::actImportRounds() {
	const std::filesystem::path file = dialog::openFile(dialog::FileTypes::JSON, true);
	if (file.empty())
		return;
...
void ConfigGameRounds::actExportRounds() const {
	const std::filesystem::path file = dialog::openFile(dialog::FileTypes::JSON, false);
	if (file.empty())
		return;
	gameEvent.exportJSON(file);
}

void ConfigGameRounds::actCancel() { reject(); }

void ConfigGameRounds::preExec() { updateDisplay(); }
...
void ConfigGameRounds::actChangePriceValue() { actEndEditingPrice(); }
...
void ConfigGameRounds::actChangeSelectedGameRound() { updateDisplay(); }

void ConfigGameRounds::actChangeSelectedSubGameRound() { updateDisplay(); }
...
void ConfigGameRounds::updateDisplayResults() {
	...
	for (auto ii = round->beginSubRound(); ii != round->endSubRound(); ++ii) {
		...
		resultats += "  grille " + QString::fromStdString(ii->getWinner()) +
					 " gagne: " + QString::fromUtf8(ii->getPrices()) + "\n";
		++i;
	}
	ui->ListWinners->setText(resultats);
}

auto ConfigGameRounds::getRoundTypes() -> QStringList {
	return {"Un quine", "Deux quines", "Carton plein", "Un quine et carton plein",
			"Normale",  "Enfants",     "Inverse",      "Pause"};
}

auto ConfigGameRounds::getVictoryType(const core::GameRound::Type& t) -> QStringList {
	switch (t) {
		...
		case core::GameRound::Type::Invalid:
			return {"none"};
	}
	return {};
}

auto ConfigGameRounds::getVictoryIndex(const core::GameRound::Type& t, const core::SubGameRound::Type& st) -> int {
	switch (t) {
		case core::GameRound::Type::OneQuineFullCard:
			switch (st) {
				...
				case core::SubGameRound::Type::Invalid:
					return -1;
			}
			return -1;
		case core::GameRound::Type::OneTwoQuineFullCard:
