...
std::string GridCard::asString() const {
    ...
    for(auto& line: lines) {
        for(auto& num: line) {
            oss << static_cast<int32_t>(num.number) << ";";
        }
        oss << ";";
    }
    return oss.str();
...
void GridCard::fromString(const std::string& s) {
    ...
    if(result.size() < 1 + nb_ligne * (nb_colones + 1))
        throw std::exception();
    pos     = 0;
    IdNumber= static_cast<uint32_t>(std::atoi(result[pos].c_str()));
    pos+= 2;
    for(auto& line: lines) {
        for(auto& num: line) {
            num.number= static_cast<uint8_t>(std::atoi(result[pos].c_str()));
            ++pos;
        }
        ++pos;
    }
...
void GridCard::read(std::istream& is, int) {
    is.read(reinterpret_cast<char*>(&IdNumber), 4);
    is.read(reinterpret_cast<char*>(&res), sizeof(Status));
    for(std::array<GCase, nb_colones>& line: lines) {
        for(GCase& _num: line) {
            is.read(reinterpret_cast<char*>(&_num), sizeof(GCase));
        }
    }
}

void GridCard::write(std::ostream& os) const {
    os.write(reinterpret_cast<const char*>(&IdNumber), 4);
    os.write(reinterpret_cast<const char*>(&res), sizeof(Status));
    for(const std::array<GCase, nb_colones>& line: lines) {
        for(const GCase& _num: line) {
            os.write(reinterpret_cast<const char*>(&_num), sizeof(GCase));
        }
    }
}
